...
import { analyticsDBUrl } from '../config';

// TODO: Customize database connection settings
/* jscs:disable requireCamelCaseOrUpperCaseIdentifiers */
db.defaults.ssl = false;
db.defaults.poolSize = 2; // TODO: Change in production
db.defaults.application_name = 'ODA';
...
db.connect = (connect => callback => new Promise((resolve, reject) => {
  connect.call(db, analyticsDBUrl, (err, client, done) => {
    if (err) {
      if (client) {
        done(client);
      }
